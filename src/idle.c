	...
	*Pending = FALSE;
		...
		sizeof(HID_SUBMIT_IDLE_NOTIFICATION_CALLBACK_INFO))
	{
		...
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_HID,
			"Error: Input buffer is too small to process idle request - STATUS:%X",
			status);
	...
	if (idleCallbackInfo == NULL || idleCallbackInfo->IdleCallback == NULL)
	{
		...
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_HID,
			"Error: Idle Notification request %p has no idle callback info - STATUS:%X",
			Request,
			status);
		goto exit;
	}

	{
		...
		if (!NT_SUCCESS(status)) {
			Trace(
				TRACE_LEVEL_ERROR,
				TRACE_FLAG_HID,
				"Error creating creating idle work item - STATUS:%X",
				status);
			goto exit;
		}
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		...
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_IDLE,
			"Error forwarding idle notification Request:0x%p to IdleQueue:0x%p - STATUS:%X",
			idleWorkItemContext->FxRequest,
			deviceContext->IdleQueue,
			status);
	...
	else
	{
		Trace(
			...
			TRACE_FLAG_IDLE,
			"Forwarded idle notification Request:0x%p to IdleQueue:0x%p - STATUS:%X",
			idleWorkItemContext->FxRequest,
			deviceContext->IdleQueue,
			status);
	}
...
--*/
{
	...
	if (!NT_SUCCESS(status) || (request == NULL))
	{
		Trace(
			TRACE_LEVEL_WARNING,
			TRACE_FLAG_IDLE,
			"Error finding idle notification request in IdleQueue:0x%p - STATUS:%X",
			FxDeviceContext->IdleQueue,
			status);
	}
	else
	{
		...
		Trace(
			TRACE_LEVEL_INFORMATION,
			TRACE_FLAG_IDLE,
			"Completed idle notification Request:0x%p from IdleQueue:0x%p - STATUS:%X",
			request,
			FxDeviceContext->IdleQueue,
			status);
	}
