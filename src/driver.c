...
--*/
{
    ...
    parse(testStr, buff);
    for(int i = 0; i < 4;i++)
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "FT: %u\n",buff[i]);*/

	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "FT: Welcome To FocalTech\n");
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"Error creating WDF driver object - STATUS:%X",
			status);
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"WdfDeviceCreate failed - STATUS:%X",
			status);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"Error creating WDF default queue - STATUS:%X",
			status);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"Error creating WDF read request queue - STATUS:%X",
			status);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"Error creating WDF idle request queue - STATUS:%X",
			status);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_INIT,
			"Error creating WDF interrupt object - STATUS:%X",
			status);
