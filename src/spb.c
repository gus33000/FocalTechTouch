	...
	if (length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		if (!NT_SUCCESS(status))
		{
			Trace(
				TRACE_LEVEL_ERROR,
				TRACE_FLAG_SPB,
				"Error allocating memory for Spb write - STATUS:%X",
				status);
			goto exit;
		}
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error setting address pointer for Spb read - STATUS:%X",
			status);
		goto exit;
	}
	...
	if (Length > DEFAULT_SPB_BUFFER_SIZE)
	{
		...
		if (!NT_SUCCESS(status))
		{
			Trace(
				TRACE_LEVEL_ERROR,
				TRACE_FLAG_SPB,
				"Error allocating memory for Spb read - STATUS:%X",
				status);
			goto exit;
		}
		...
		bytesRead != Length)
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error reading from Spb - STATUS:%X",
			status);
		goto exit;
	}
...
--*/
{
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error creating IoTarget object - STATUS:%X",
			status);
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error creating Spb resource hub path string - STATUS:%X",
			status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error opening Spb target for communication - STATUS:%X",
			status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error allocating default memory for Spb write - STATUS:%X",
			status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error allocating default memory for Spb read - STATUS:%X",
			status);
		goto exit;
	}
	...
	if (!NT_SUCCESS(status))
	{
		Trace(
			TRACE_LEVEL_ERROR,
			TRACE_FLAG_SPB,
			"Error creating Spb Waitlock - STATUS:%X",
			status);
		goto exit;
	}
